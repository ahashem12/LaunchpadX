...
export function RoleHeader({ role, roleType, roleCategory }: RoleHeaderProps) {
  ...
  return (
    ...
    >
      ...
      {hasBanner ? (
        <div className="w-full h-full bg-gradient-to-r from-green-900 to-green-600" />
      ) : (
        <>
          {/* Pigeon Icons */}
      ...
      <div className="absolute bottom-4 left-6 right-6">
        <div className="flex items-end justify-between">
            ...
            <div className="flex flex-col">
              ...
              <div className="flex items-center gap-4 mt-2">
                ...
                <span className="text-xs text-white/80">
                  Opened:{" "}
                  {role.opened_at
                    ? new Date(role.opened_at).toLocaleDateString()
                    : "-"}
                </span>
                <span className="text-xs text-white/80">
                  Closed:{" "}
                  {role.closed_at
                    ? new Date(role.closed_at).toLocaleDateString()
                    : "-"}
                </span>
              </div>
            </div>
          ...
          <div className="flex-shrink-0">
            <HeaderApplyButton roleId={role.id} />
          </div>
        </div>
      </div>
    </div>
