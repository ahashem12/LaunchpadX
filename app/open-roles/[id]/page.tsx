...
import { ApplyButton } from "@/components/open-roles/details/ApplyButton";
...
export default async function DetailsPage(props: DetailsPageProps) {
  ...
  const skills = await projectService.getSkillsByIds(
    role.required_skills || []
  );
  const project = role.project_id
    ? await projectService.getProject(role.project_id)
    : null;
  const teamMembers = project
    ? await projectService.getTeamMembers(project.id)
    : [];
  ...
  return (
    <div className="space-y-6">
      ...
      {project && (
        <ProjectRoleSection project={project} teamMembers={teamMembers} />
      )}
