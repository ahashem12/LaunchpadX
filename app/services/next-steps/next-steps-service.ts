...
import { validateProjectId } from "../projects/utils"
...
export class NextStepsService {
  ...
  try {
    if (!validateProjectId(input.project_id)) {
  ...
  async updateNextStep(id: string, input: UpdateNextStepInput): Promise<NextStep | null> {
    try {
      const { data, error } = await this.supabase
        .from("next_steps")
        .update({
  ...
  async deleteNextStep(id: string): Promise<boolean> {
    try {
      const { error } = await this.supabase.from("next_steps").delete().eq("id", id)
  ...
  async toggleNextStepDone(id: string, done: boolean): Promise<NextStep | null> {
